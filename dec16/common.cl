
(defconstant +north+ 0)
(defconstant +east+ 1)
(defconstant +south+ 2)
(defconstant +west+ 3)

(defparameter *input* (make-array 1 :adjustable t :fill-pointer 0))
(defparameter *height* 0)

(defun readFile (fname)
  (progn
(with-open-file (stream fname)
  ( loop for line = (read-line stream nil 'eof)
         until (eq line 'eof)
         do (vector-push-extend line *input* )
         do (setf *height* (+ 1 *height*))))
(defparameter *width* (length (aref *input* 0)))
(defparameter *field* (make-array `(,*width* ,*height*) :initial-element #\.))
(defparameter *visited* (make-array `(,*width* ,*height*) :initial-element nil))
(defparameter *cache* (make-array `(,*width* ,*height* ,4) :initial-element nil))
(defparameter *dirstack* '())
(loop for y below *height*
      do ( loop for x below *width*
                do ( setf (aref *field* x y) (char (aref *input* y) x))))
))

(defun incx (x direction)
  (ccase direction
         (#.+north+ x)
         (#.+south+ x)
         (#.+east+ (+ x 1))
         (#.+west+ (- x 1))
         )
  )

(defun incy (y direction)
  (ccase direction
         (#.+north+ (- y 1))
         (#.+south+ (+ y 1))
         (#.+east+ y)
         (#.+west+ y)
         )
  )

(defun move (x y direction)
  (list (incx x direction) (incy y direction) direction)
  )
(defun isvert (direction)
  (ccase direction
         (#.+north+ t)
         (#.+south+ t)
         (#.+east+ nil)
         (#.+west+ nil)
         )
  )

(defun dirslash (direction)
  (ccase direction
         (#.+north+ +east+)
         (#.+south+ +west+)
         (#.+east+ +north+)
         (#.+west+ +south+)
         )
  )

(defun dirback (direction)
  (ccase direction
         (#.+north+ +west+)
         (#.+south+ +east+)
         (#.+east+ +south+)
         (#.+west+ +north+)
         )
  )
(defun needsWork (x y direction)
  (and
    (>= x 0)
    (>= y 0)
    (< x *width*)
    (< y *height*) ;; maybe of by one
    (not (aref *cache* x y direction))
    )
  )

(defun doStep (x y direction)
  (progn
    (when (needsWork x y direction)
      (setf (aref *cache* x y direction) t)
      (setf (aref *visited* x y) t)
      (ccase (aref *field* x y)
             (#\. (push  (move x y direction)  *dirstack*))
             (#\- (if (isvert direction)
                    (progn
                      (push (move x y +east+) *dirstack* )
                      (push (move x y +west+) *dirstack* )
                      )
                    (push (move x y direction) *dirstack* )
                    ))
             (#\| (if (isvert direction)
                    (push (move x y direction) *dirstack* )
                    (progn
                      (push (move x y +south+) *dirstack*)
                      (push (move x y +north+) *dirstack*)
                      )
                    ))
             (#\/ (push (move x y (dirslash direction)) *dirstack*))
             (#\\ (push (move x y (dirback direction)) *dirstack*))
             ))))

(defun tryStart (start)
  (progn
    (push start *dirstack*)
    (aref *cache* 0 0 +east+ )
    (loop while *dirstack*
          do ( progn
               ( setq nextStep (pop *dirstack*))
               (apply #'doStep nextStep)))
    (setq result (loop for y below *height*
          sum ( loop for x below *width*
                     sum ( if (aref *visited* x y ) 1 0)))
    )
  (reset)
  result))


(defun reset ()
  (progn
  (setf *visited* (make-array `(,*width* ,*height*) :initial-element nil))
  (setf *cache* (make-array `(,*width* ,*height* ,4) :initial-element nil))
  )
)

